...
export default function ImovelCard({ imovel }: { imovel: Imovel }) {
  ...
  return (
    <Link
      ...
      className="group block overflow-hidden rounded-lg border border-border bg-card shadow-sm transition-shadow hover:shadow-md active:scale-[0.99]"
    >
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-surface">
        ...
        {imovel.aceitaPermuta && (
          <span className="absolute left-2.5 top-2.5 inline-flex items-center gap-1 rounded-md bg-primary/80 px-2 py-0.5 text-[11px] font-semibold text-white shadow-sm backdrop-blur-md ring-1 ring-white/10">
        ...
        {imovel.fotos.length > 1 && (
          <span className="absolute right-2.5 top-2.5 inline-flex items-center gap-1 rounded-md bg-primary/80 px-2 py-0.5 text-[11px] font-medium text-white shadow-sm backdrop-blur-md ring-1 ring-white/10">
      ...
      <div className="p-4">
        <div className="flex items-baseline justify-between gap-2">
          <p className="text-xs uppercase tracking-wide text-muted font-medium">{imovel.tipo}</p>
          <p className="text-[11px] text-muted">{formatArea(imovel.area)}</p>
        </div>
        <h3 className="mt-1 font-semibold text-sm leading-snug line-clamp-2 text-foreground">
          {imovel.titulo}
        </h3>
        ...
        <div className="mt-3 flex items-end justify-between gap-2">
          <span className="text-base font-bold text-foreground">{formatBRL(imovel.valor)}</span>
